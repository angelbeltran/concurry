...
var (
	...
	normalizedOutputTypeName string
)
...
func concurryCmdRunfunc(cmd *cobra.Command, args []string) error {
	...
	if normalizedOutputFileName == "" {
		normalizedOutputFileName = getDefaultOutputFilename()
	}

	// validate 'name'
	normalizedOutputTypeName = outputTypeName
	if normalizedOutputTypeName == "" {
		normalizedOutputTypeName = sourceTypeName + "WithContext"
	...
	file, err := os.Create(normalizedOutputFileName)
	if err != nil {
	...
	command := make([]string, 3, 7)
	command[0] = "concurry"
	command[1] = "-s"
	command[2] = sourceTypeName
	if outputFileName != "" {
		command = append(command, "-o", outputFileName)
	}
	if outputTypeName != "" {
		command = append(command, "-n", outputTypeName)
	}
...
`,
	...
	methodsBuf, methodImports := generateOutputTypeMethods(p.PkgPath, normalizedOutputTypeName, namedType)
	...
	decBuf := generateTypeDeclarationAndConstructor(normalizedOutputTypeName)
...
func generateTypeDeclarationAndConstructor(normalizedOutputTypeName string) []byte {
	isOutputTypeExported := strings.ToUpper(normalizedOutputTypeName[0:1]) == normalizedOutputTypeName[0:1]
	...
	constructorName := fmt.Sprintf("%s%s", termNew, capitalize(normalizedOutputTypeName))
...
func generateOutputTypeMethods(pkgPath, normalizedOutputTypeName string, namedType *types.Named) (buf []byte, imports []string) {
